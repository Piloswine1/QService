...
const getLatLonByName = async (city: string) => {
    const params = new URLSearchParams ({
        q: city.toLowerCase(),
        appid: Deno.env.get('WEATHER_API_APPID') ?? ''
    });
    const res = await fetch(`${Deno.env.get('WEATHER_GEOCODING_API')}/direct?${params.toString()}`, {
        method: 'GET'
    });
    if (res.status !== 200)
...
export class CurrentWeatherResource extends BaseResource {
    ...
    public async GET(request: Drash.Request, response: Drash.Response) {
        ...
        const res = await fetch(`${Deno.env.get('WEATHER_API')}/weather?${params.toString()}`, {
            method: 'GET'
        });
        if (res.status !== 200)
            throw new Error("failed");
